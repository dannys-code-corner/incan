...
pub fn run_tests(
    ...
    if test_files.is_empty() {
        ...
        eprintln!("Test files should be named test_*.incn or *_test.incn");
        process::exit(1);
    }
    ...
    if verbose && !all_fixtures.is_empty() {
        ...
        for (name, fixture) in &all_fixtures {
            ...
            println!("  - {}: scope={}{}{}{}", name, scope_str, autouse_str, async_str, teardown_str);
        }
        println!();
    }
    ...
    let all_tests: Vec<TestInfo> = all_tests.into_iter().map(|mut test| {
        ...
        let result = if is_xfail {
            match result {
                ...
                TestResult::Failed(d, _) => {
                    let reason = test.markers.iter()
                        .find_map(|m| if let TestMarker::XFail(r) = m { Some(r.clone()) } else { None })
                        .unwrap_or_default();
                    xfailed += 1;
    ...
    println!();
    let summary_color = if failed > 0 || xpassed > 0 { "\x1b[1;31m" } else { "\x1b[1;32m" };
    print!("{}===================", summary_color);
    ...
    if xfailed > 0 { parts.push(format!("{} xfailed", xfailed)); }
    if xpassed > 0 { parts.push(format!("{} xpassed", xpassed)); }
...
fn print_test_result(test: &TestInfo, result: &TestResult, verbose: bool) {
    let file_stem = test.file_path.file_name()
        .and_then(|n| n.to_str())
        .unwrap_or("unknown");
...
pub fn discover_tests_and_fixtures(file_path: &Path) -> Result<DiscoveryResult, String> {
    ...
    let fixture_names: Vec<String> = ast.declarations.iter()
        .filter_map(|decl| {
...
fn extract_fixture_args(decorators: &[crate::frontend::ast::Spanned<crate::frontend::ast::Decorator>]) -> (FixtureScope, bool) {
    ...
    for dec in decorators {
        if dec.node.name == "fixture" {
            for arg in &dec.node.args {
                match arg {
                    crate::frontend::ast::DecoratorArg::Named(name, value) => {
                        if name == "scope" {
                            ...
                            if let crate::frontend::ast::DecoratorArgValue::Expr(expr) = value {
                                if let crate::frontend::ast::Expr::Literal(crate::frontend::ast::Literal::Bool(b)) = &expr.node {
                                    autouse = *b;
                                }
                            }
                        }
    ...
    match stmt {
        ...
        crate::frontend::ast::Statement::If(if_stmt) => {
            if_stmt.then_body.iter().any(|s| statement_has_yield(&s.node))
                || if_stmt.else_body.as_ref().map_or(false, |b| b.iter().any(|s| statement_has_yield(&s.node)))
        }
...
fn expr_has_yield(expr: &crate::frontend::ast::Expr) -> bool {
    match expr {
        ...
        crate::frontend::ast::Expr::Call(callee, args) => {
            expr_has_yield(&callee.node) || args.iter().any(|a| match a {
...
fn extract_test_markers(decorators: &[crate::frontend::ast::Spanned<crate::frontend::ast::Decorator>]) -> Vec<TestMarker> {
...
fn extract_string_arg(args: &[crate::frontend::ast::DecoratorArg]) -> Option<String> {
    if let Some(arg) = args.first() {
        if let crate::frontend::ast::DecoratorArg::Positional(expr) = arg {
            if let crate::frontend::ast::Expr::Literal(crate::frontend::ast::Literal::String(s)) = &expr.node {
                return Some(s.clone());
            }
        }
...
fn run_single_test(test: &TestInfo) -> TestResult {
    ...
    let source = match fs::read_to_string(&test.file_path) {
        Ok(s) => s,
        Err(e) => return TestResult::Failed(start.elapsed(), format!("Failed to read file: {}", e)),
    };
