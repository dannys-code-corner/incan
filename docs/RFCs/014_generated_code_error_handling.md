...
|-----------------------|---------|-----------------------------------|
| `d[key]`              | `.get(...).expect("KeyError: key not found in dict").clone()` *(string-literal keys include the key in the message)* | `.get(...).unwrap_or_else(\|\| panic!("KeyError: key not found in dict")).clone()` |
| `d.get(key)`          | N/A     | `.get(&k).cloned()` → `Option<V>` |
| `d.get(key, default)` | N/A     | `.get(&k).cloned().unwrap_or(default)` |

### String Parsing

| Operation  | Current             | Proposed          |
|------------|---------------------|-------------------|
| `int(s)`   | **Depends on input form:** string literal → `.parse::<i64>().expect("ValueError: ...")`; known `String` var → `.parse::<i64>().unwrap_or_else(\|_\| panic!("ValueError: ...", s))`; non-string → `(x as i64)` | Parse strings with a `ValueError` message; cast non-strings for performance |
| `float(s)` | **Depends on input form:** string literal → `.parse::<f64>().expect("ValueError: ...")`; known `String` var → `.parse::<f64>().unwrap_or_else(\|_\| panic!("ValueError: ...", s))`; non-string → `(x as f64)` | Parse strings with a `ValueError` message; cast non-strings for performance |

### List Operations

| Operation       | Current  | Proposed          |
|-----------------|----------|-------------------|
| `list[i]`       | `vec[(i as usize)].clone()` *(panics with Rust bounds message today)* | `vec.get(i as usize).cloned().unwrap_or_else(\|\| panic!("IndexError: index {} out of range for list of length {}", i, vec.len()))` |
| `list.index(v)` | `__incan_list_find_index(&vec, &v).expect("ValueError: value not found in list")` | `__incan_list_find_index(&vec, &v).unwrap_or_else(\|\| panic!("ValueError: value not found in list"))` |

## Implementation Plan

1. **Phase 1**: Update codegen to emit better panic messages
2. **Phase 2**: Add IncanError type to generated prelude
3. **Phase 3**: Add `Dict.get(key, default=...)` **and introduce a new safe list API** (e.g. `List.get(index, default=...)` or `List.at(index)` returning `Option[T]`) and update codegen to use Rust's `Vec::get`.
