...
Incan provides helpful, "Python-friendly" error messages that explain Rust concepts in familiar terms.
...
# Wrong
def greet() -> str:
    let name = get_name()  # Returns Result[str, Error]
    return name  # ❌ name is still Result

# Right - use ? to unwrap
def greet() -> Result[str, Error]:
    let name = get_name()?  # ✅ Unwraps Ok, returns Err early
...
3. Look at [Error Handling](error_handling.md) for `Result`/`Option` patterns
4. See [Derives & Traits](derives_and_traits.md) for trait requirements
