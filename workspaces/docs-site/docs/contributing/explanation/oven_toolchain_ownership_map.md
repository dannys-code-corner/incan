...
- [ ] One linked acceptance path per responsibility — **partially done**. The confirmed-kernel and compiler-service-exception rows don't need one (no migration implied). The migration-candidate and uncertain rows all currently point to #1034 as the umbrella; fine-grained per-component delivery issues should be split out from #1034 once this map is accepted, not invented here.
