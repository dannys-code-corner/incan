...
## v0.5.1-dev.21: final patch candidate

`v0.5.1-dev.21` is the final development candidate for Incan's first patch release. It is not a release candidate and does not add a new language or product surface. It is the validation vehicle for the focused repairs tracked under [#1191](https://github.com/encero-systems/incan/issues/1191).
...
- An explicit Incan script that was not included in an Oven project bake now names itself as the missing executable target and explains that it must be declared under `[project.scripts]` before rebaking, instead of incorrectly reporting missing dependencies with `Needs: none`. This additional bounded repair is recorded in the #1191 parent alongside its nine linked defects.

The repeat-`incan oven bake` authority failure in [#1194](https://github.com/encero-systems/incan/issues/1194) is represented here as repaired: the format-1-to-format-2 migration and an unchanged rebake pass under the current control. The exact RC candidate must repeat that control before a release decision.
