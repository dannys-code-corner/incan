...
## v0.5.1-dev.2: targeted patch validation

`v0.5.1-dev.2` is a development build for Incan's first patch release. It is not a release candidate and does not add a new language or product surface. It is the validation vehicle for the focused repairs tracked under [#1191](https://github.com/encero-systems/incan/issues/1191).

Validated repairs in this development build:
...
- A compiler built from source can seal its vocabulary-helper closure at the explicit `incan oven bake` boundary; normal build paths consume that sealed result without gaining Cargo authority.
- Oven source-authority selection recognises the same sealed registry source archive across compatible feature selections, an initial explicit project bake may establish its first Cargo lock, and aggregate-only storage limits no longer fail because of an inherited per-domain default.
...
The reported repeat-`incan oven bake` authority failure remains under investigation in [#1194](https://github.com/encero-systems/incan/issues/1194). It has not been reproduced by the current control test, so it is not represented here as fixed. The patch train will not advance to a release candidate until that result is resolved or conclusively falsified.
