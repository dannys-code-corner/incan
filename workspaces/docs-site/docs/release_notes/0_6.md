...
- **Tooling**: Experimental replacement execution admits compiler-selected `str`, `int`, and `float` scalar conversions with receipt-bound normal output and classified parse failures. Runtime numeric strings currently inherit a parser inconsistency for underscore separators, tracked by #1278. This is not full numeric execution: #1279 owns the typed carrier needed for width-specific and decimal values, while #988 retains the broader float-operation and entrypoint profile. (#1249)
