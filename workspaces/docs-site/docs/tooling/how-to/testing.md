...
When a generated Rust test harness is new or its compatibility inputs change, `incan test` writes a new receipt and selects the matching prepared Oven plan. A missing plan fails with its generated harness and receipt path, then explains whether to install an Oven-enabled toolchain or remove caller-owned Rust dependencies outside the documented Alpha envelope. The diagnostic refers maintainers to the internal compatibility publisher only when preparing a toolchain; `incan test` neither runs it automatically nor falls back to Cargo.
