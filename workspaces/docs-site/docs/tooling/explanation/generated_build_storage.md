...
Before Oven Alpha, repository CI supplied `INCAN_GENERATED_CARGO_TARGET_DIR`, transferred Nextest archives, and split the suite into Cargo-backed shards. The measurements below explain why that topology was selected at the time; they are not the current #1005 CI architecture. Current stable Linux, stable macOS, and Rust 1.93 lanes invoke the single compiler-suite Loaf baker and then run the complete prepared suite through the Cargo-guarded Oven runner beside its bounded store.
