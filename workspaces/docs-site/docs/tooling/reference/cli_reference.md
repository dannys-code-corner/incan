...
- `--backend <legacy|replacement>`: Declare the compiler backend for this build (#986). Defaults to `legacy`, declared explicitly even when the flag is omitted. `replacement` builds one session-owned analysis for the selected entry and package-feature projection, then directly executes the admitted zero-argument `main` Body-IR profile and its permitted same-module calls. Package and public imports, aggregates, repeated user-binding names (shadowing or reassignment), and other source outside that bounded #988 profile still refuse visibly. It does not create generated Rust or an Oven plan. See [Backend selection & execution receipts](../explanation/backend_selection_receipts.md).
