...
- `--backend <legacy|replacement>`: Declare the compiler backend for this build (#986). Defaults to `legacy`, declared explicitly even when the flag is omitted. `replacement` builds one session-owned analysis for the selected entry and package-feature projection, then directly executes the admitted zero-argument `main` Body-IR profile and its permitted same-module calls. Package and public imports, unsupported aggregate shapes, repeated user-binding names (shadowing or reassignment), and other source outside that bounded #988 profile still refuse visibly. It does not create generated Rust or an Oven plan. See [Backend selection & execution receipts](../explanation/backend_selection_receipts.md).
...
- `--report json`: Emit a versioned machine-readable build report. The session-owned direct replacement path emits its own `incan.replacement_execution.v0` report because it has no generated Rust, native artifact, or Oven plan; its top-level `semantic_module` identifies the selected checked module and its source and semantic-snapshot identities.
