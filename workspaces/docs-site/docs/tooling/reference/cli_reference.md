...
`bake` is the project-facing preparation command. It records debug and release receipts, reports `toolchain_loaf` when a profile selects the immutable full-stdlib Loaf directly, and reports `reused` on an exact warm project extension. A project that needs artifacts outside the installed standard-library envelope may explicitly invoke the bounded compatibility baker once. The resulting `<identity>.loaf/loaf.json` replaces compiler-owned runtime artifacts with the exact base copies, retains the remaining project-specific fragment, and names that base Loaf; normal execution leases and composes both verified roots. It never compiles a final library, and normal build/test/run never invoke Cargo or use a fallback.
