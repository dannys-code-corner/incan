...
For the documented Oven Alpha envelope, current beta `incan build`, `incan test`, and `incan run` select a receipt-bound direct-`rustc` plan from Incan-owned storage rather than invoking Cargo. Generated Rust remains useful, inspectable backend output, while the explicitly named `legacy_cargo` Loaf baker is the sole Cargo-backed compatibility publisher rather than a normal-command backend.
