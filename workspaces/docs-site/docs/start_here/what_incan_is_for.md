...
Incan is beta software. In the documented Oven Alpha envelope, ordinary build, test, and run commands select receipt-bound direct-`rustc` plans from Incan-owned storage; Cargo is confined to a named temporary publisher rather than acting as a normal-command fallback. Generated Rust remains inspectable backend output, but it should not be treated as the permanent public semantic contract.
