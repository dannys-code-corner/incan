...
function usage() {
  console.log(`Prepare the npm toolchain package.

The published package is a reference shim: command entry points plus the bundled
installer that provisions the verified release archive on first invocation. No
toolchain payload is packed into npm, keeping the upload within registry size
limits and matching the pip package's delivery model.
