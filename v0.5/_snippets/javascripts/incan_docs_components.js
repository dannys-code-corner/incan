...
(function () {
  ...
  function setupVersionIdentity() {
    const isLocalPreview = ["127.0.0.1", "localhost"].includes(window.location.hostname);
    const isDevelopmentDocs = isLocalPreview || /(^|\/)dev(\/|$)/.test(window.location.pathname);
    if (!isDevelopmentDocs) return;

    document.querySelectorAll(".md-source__fact--version").forEach((version) => {
      version.textContent = "dev docs";
      version.setAttribute("aria-label", "Development documentation");
      version.title = "Development documentation built from the current main branch";
    });
  }
